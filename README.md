...
project.html            detail page — wireframe 4a / 4b; the template for the pages below
<slug>.html             one per project (formel.html, louvo.html) — generated, served as /<slug>
...
tools/build-pages.js    writes <slug>.html for every project
...
(`rodarly.me/your-slug`), the home page grows a card, and the "next project"
link at the foot of each detail page rotates through the list.

Then write its page and commit it:

```sh
node tools/build-pages.js
```

That copies `project.html` to `your-slug.html` with the project's title and description
filled in, and deletes pages for slugs that no longer exist. GitHub Pages, Netlify and
Cloudflare Pages serve `/your-slug` from `your-slug.html` with no setup (on Vercel, set
`"cleanUrls": true`). Re-run it whenever `project.html` changes too. Local servers don't
drop the `.html`, so on `localhost` the links point at `your-slug.html` instead; old
`project.html?p=your-slug` links still work and switch to the clean address.
